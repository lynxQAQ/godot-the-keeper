...
# GridSystemSurface：协调两者 + 处理输入交互（表世界专用）
...
# ========== 生命周期 ==========
func _ready():
	# 确保 GridSystemSurface 节点的 position 为 (0,0)，避免坐标偏移
...
func _is_mouse_in_viewport() -> bool:
	...
	if parent is Control:
		# 如果 GridSystemSurface 是 Control 的子节点，检查鼠标是否在 Control 的 rect 内
		...
		# 如果 GridSystemSurface 不是 Control 的子节点，检查鼠标是否在表世界视口内
		var viewport = get_viewport()
		if not viewport:
			return false
		# 检查是否是表世界的 SubViewport
		if viewport.name == "SubViewport_Surface":
